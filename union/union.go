...
#define UNION_LIBNAME "libunionpreload.so"
#define LD_PRELOAD "LD_PRELOAD"
#define UNION_PRELOAD "UNION_PRELOAD"
static char **saved_ld_preloads = NULL;
static size_t num_saved_ld_preloads = 0;
static char *saved_union_preload = NULL;
static char *saved_tmpdir = NULL;
static char *saved_varlib = NULL;

static void constructor() __attribute__((constructor));

static char *getenvdup(const char *varname) {
...
void constructor() {
  ...
  // We need to save LD_PRELOAD and UNION_PRELOAD in case we need to
  ...
  saved_union_preload = getenvdup(UNION_PRELOAD);
  if (saved_union_preload == NULL) {
    free(ld_preload_copy);
    return;
  }

  saved_tmpdir = getenvdup("UNION_APP_TMPDIR");
  if (!saved_tmpdir) {
    saved_tmpdir = getenvdup("TMPDIR");
  }

  saved_varlib = getenvdup("UNION_APP_DATA_PATH");
  if (!saved_varlib) {
    saved_varlib = getenvdup("UNION_APP_DATA_PATH");
  }

  // Pull out each absolute-pathed libunionpreload.so we find.  Better to
  // accidentally include some other libunionpreload than not propagate
  // ourselves.
  libnamelen = strlen(UNION_LIBNAME);
  for (p = strtok_r(ld_preload_copy, " :", &savedptr); p;
       p = strtok_r(NULL, " :", &savedptr)) {
    size_t plen = strlen(p);
    if (plen > libnamelen && p[0] == '/' &&
        strcmp(p + strlen(p) - strlen(UNION_LIBNAME) - 1,
               "/" UNION_LIBNAME) == 0) {
      num_saved_ld_preloads++;
      saved_ld_preloads = realloc(saved_ld_preloads,
                                  (num_saved_ld_preloads + 1) * sizeof(char *));
...
static char *redirect_writable_path(const char *pathname,
                                    const char *basepath) {
  ...
  strncat(redirected_pathname, pathname,
          PATH_MAX - 1 - strlen(redirected_pathname));

  // No need to see if it already exists -- app can only be in TMPDIR, not /tmp
  return redirected_pathname;
}

static char *redirect_path_full(const char *pathname, int check_parent,
                                int only_if_absolute) {
  ...
  preload_dir = saved_union_preload;
  if (preload_dir == NULL) {
  ...
  if (strcmp(pathname, "/tmp") == 0 || strncmp(pathname, "/tmp/", 5) == 0) {
    if (saved_tmpdir &&
        strncmp(pathname, saved_tmpdir, strlen(saved_tmpdir)) != 0) {
  ...
  if (strcmp(pathname, "/var/lib") == 0 ||
      strncmp(pathname, "/var/lib/", 9) == 0) {
    if (saved_varlib &&
        strncmp(pathname, saved_varlib, strlen(saved_varlib)) != 0 &&
        _access(pathname, F_OK) != 0) {
  ...
  if (ret == 0 ||
      errno == ENOTDIR) {  // ENOTDIR is OK because it exists at least
    return redirected_pathname;
  } else {
    free(redirected_pathname);
    return strdup(pathname);
...
static char *redirect_path(const char *pathname) {
  return redirect_path_full(pathname, 0, 0);
}

static char *redirect_path_target(const char *pathname) {
  return redirect_path_full(pathname, 1, 0);
}

static char *redirect_path_if_absolute(const char *pathname) {
  return redirect_path_full(pathname, 0, 1);
}

#define REDIRECT_1_1(RET, NAME)                                \
  RET NAME(const char *path) {                                 \
...
#define REDIRECT_1_2(RET, NAME, T2)                                   \
  RET NAME(const char *path, T2 A2) {                                 \
...
#define REDIRECT_1_3(RET, NAME, T2, T3)                                      \
  RET NAME(const char *path, T2 A2, T3 A3) {                                 \
...
#define REDIRECT_2_2(RET, NAME, T1)                                   \
  RET NAME(T1 A1, const char *path) {                                 \
...
#define REDIRECT_2_3(RET, NAME, T1, T3)                                      \
  RET NAME(T1 A1, const char *path, T3 A3) {                                 \
...
#define REDIRECT_2_4_AT(RET, NAME, T1, T3, T4)                              \
  RET NAME(T1 A1, const char *path, T3 A3, T4 A4) {                         \
    RET (*_NAME)(T1 A1, const char *path, T3 A3, T4 A4);                    \
    char *new_path = NULL;                                                  \
    RET result;                                                             \
    _NAME = (RET(*)(T1 A1, const char *path, T3 A3, T4 A4))dlsym(RTLD_NEXT, \
                                                                 #NAME);    \
    new_path = redirect_path_if_absolute(path);                             \
...
#define REDIRECT_3_5(RET, NAME, T1, T2, T4, T5)                          \
  RET NAME(T1 A1, T2 A2, const char *path, T4 A4, T5 A5) {               \
    RET (*_NAME)(T1 A1, T2 A2, const char *path, T4 A4, T5 A5);          \
    char *new_path = NULL;                                               \
    RET result;                                                          \
    _NAME = (RET(*)(T1 A1, T2 A2, const char *path, T4 A4, T5 A5))dlsym( \
        RTLD_NEXT, #NAME);                                               \
    new_path = redirect_path(path);                                      \
...
#define REDIRECT_TARGET(RET, NAME)                                             \
  RET NAME(const char *path, const char *target) {                             \
    RET (*_NAME)(const char *path, const char *target);                        \
    char *new_path = NULL;                                                     \
    char *new_target = NULL;                                                   \
    RET result;                                                                \
    _NAME =                                                                    \
        (RET(*)(const char *path, const char *target))dlsym(RTLD_NEXT, #NAME); \
    new_path = redirect_path(path);                                            \
...
#define REDIRECT_OPEN(NAME)                                           \
  int NAME(const char *path, int flags, ...) {                        \
    ...
    _NAME = (int (*)(const char *path, int flags, mode_t mode))dlsym( \
        RTLD_NEXT, #NAME);                                            \
    new_path = redirect_path(path);                                   \
...
#define REDIRECT_OPEN_AT(NAME)                                         \
  int NAME(int dirfp, const char *path, int flags, ...) {              \
    int mode = 0;                                                      \
    ...
    _NAME = (int (*)(int dirfp, const char *path, int flags,           \
                     mode_t mode))dlsym(RTLD_NEXT, #NAME);             \
    new_path = redirect_path_if_absolute(path);                        \
...
int scandir(const char *dirp, struct dirent ***namelist,
  ...
  _scandir = (int (*)(const char *dirp, struct dirent ***namelist,
                      int (*filter)(const struct dirent *),
                      int (*compar)(const struct dirent **,
                                    const struct dirent **)))dlsym(RTLD_NEXT,
                                                                   "scandir");
...
int scandir64(const char *dirp, struct dirent64 ***namelist,
  ...
  int (*_scandir64)(
      const char *dirp, struct dirent64 ***namelist,
      int (*filter)(const struct dirent64 *),
  ...
  _scandir64 = (int (*)(
      ...
      int (*filter)(const struct dirent64 *),
      int (*compar)(const struct dirent64 **,
                    const struct dirent64 **)))dlsym(RTLD_NEXT, "scandir64");
...
int scandirat(int dirfd, const char *dirp, struct dirent ***namelist,
              ...
              int (*compar)(const struct dirent **, const struct dirent **)) {
  int (*_scandirat)(
      int dirfd, const char *dirp, struct dirent ***namelist,
      int (*filter)(const struct dirent *),
      int (*compar)(const struct dirent **, const struct dirent **));
  char *new_path = NULL;
  int ret;
  ...
  _scandirat =
      (int (*)(int dirfd, const char *dirp, struct dirent ***namelist,
               int (*filter)(const struct dirent *),
               int (*compar)(const struct dirent **, const struct dirent **)))
          dlsym(RTLD_NEXT, "scandirat");
...
int scandirat64(int dirfd, const char *dirp, struct dirent64 ***namelist,
                ...
                int (*compar)(const struct dirent64 **,
                              const struct dirent64 **)) {
  int (*_scandirat64)(
      int dirfd, const char *dirp, struct dirent64 ***namelist,
      int (*filter)(const struct dirent64 *),
      int (*compar)(const struct dirent64 **, const struct dirent64 **));
  char *new_path = NULL;
  int ret;
  ...
  _scandirat64 = (int (*)(
      ...
      int (*filter)(const struct dirent64 *),
      int (*compar)(const struct dirent64 **,
                    const struct dirent64 **)))dlsym(RTLD_NEXT, "scandirat64");
...
int bind(int sockfd, const struct sockaddr *addr, socklen_t addrlen) {
  ...
  _bind = (int (*)(int sockfd, const struct sockaddr *addr,
                   socklen_t addrlen))dlsym(RTLD_NEXT, "bind");

  if (addr->sa_family == AF_UNIX &&
      ((const struct sockaddr_un *)addr)->sun_path[0] !=
          0) {  // could be abstract socket
    char *new_path = NULL;
    ...
    result =
        _bind(sockfd, (const struct sockaddr *)&new_addr, sizeof(new_addr));
  } else {
...
int connect(int sockfd, const struct sockaddr *addr, socklen_t addrlen) {
   ...
   */
  if (addr->sa_family == AF_UNIX && addrlen > sizeof(sa_family_t) &&
    ...
    return _connect(sockfd, (const struct sockaddr *)&new_addr,
                    sizeof(new_addr));
  }
...
static char *ensure_in_ld_preload(char *ld_preload, const char *to_be_added) {
  if (ld_preload && ld_preload[0] != 0) {
    ...
    for (p = strtok_r(ld_preload_copy + strlen(LD_PRELOAD) + 1, " :",
                      &savedptr);
         p; p = strtok_r(NULL, " :", &savedptr)) {
    ...
    if (!found) {
      ld_preload =
          realloc(ld_preload, strlen(to_be_added) + strlen(ld_preload) + 2);
      strcat(ld_preload, ":");
...
static char **execve_copy_envp(char *const envp[]) {
  int i, num_elements;
  char **new_envp = NULL;
  char *ld_preload = NULL;
  char *union_preload = NULL;
  ...
  if (saved_ld_preloads) {
    ...
    for (j = 0; j < num_saved_ld_preloads; j++) {
      ld_preload_copy =
          ensure_in_ld_preload(ld_preload_copy, saved_ld_preloads[j]);
    }
    new_envp[i++] = ld_preload_copy;
  }

  if (saved_union_preload) {
    union_preload =
        malloc(strlen(saved_union_preload) + strlen(UNION_PRELOAD) + 2);
    strcpy(union_preload, UNION_PRELOAD "=");
    strcat(union_preload, saved_union_preload);
    new_envp[i++] = union_preload;
  }
...
static int execve32_wrapper(int (*_execve)(const char *path, char *const argv[],
                                           char *const envp[]),
...
static int execve_wrapper(const char *func, const char *path,
  ...
  _execve = (int (*)(const char *path, char *const argv[],
                     char *const envp[]))dlsym(RTLD_NEXT, func);
  ...
  if (result == -1 && errno == ENOENT) {
    ...
    int (*_access)(const char *pathname, int mode);
    _access =
        (int (*)(const char *pathname, int mode))dlsym(RTLD_NEXT, "access");
